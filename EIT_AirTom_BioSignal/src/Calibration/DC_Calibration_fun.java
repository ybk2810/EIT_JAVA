...
import MainFunctions.S_EIT_Doc;
...
public class DC_Calibration_fun {
	...
	int[]  m_nOverflow= new int[Definition.Serial_EIT_ALL_CH];
	int[]  m_nReal= new int[Definition.Serial_EIT_ALL_CH];
	int[]  m_nQuad= new int[Definition.Serial_EIT_ALL_CH];
	
	int[]  m_nDCOffsetValue= new int[Definition.Serial_EIT_ALL_CH];
	long[] m_nAccumulatedData = new long [Definition.Serial_EIT_ALL_CH];
	long[] m_nAccumulatedGNDData = new long[Definition.Serial_EIT_ALL_CH];
	...
	FileWriter[] fout = new FileWriter[Definition.Serial_EIT_ALL_CH];
	...
	boolean DCOffsetCalSystemSetting() throws HeadlessException, InterruptedException
		{
		    ...
		    /*if(!DCoffSet_Calibration.getEIT_Control_Dlg().EIT_Control.AnalogBackplaneSWSetting(Definition.ALL_CH,1,1,0,0,0,0)) //GND, SW2, SW3, SW4, SW5 (GND)
		 	{
		 		JOptionPane.showMessageDialog(null, "AnalogBackplane Switch Setting Error", "InfoBox: " + "Error", JOptionPane.INFORMATION_MESSAGE);
		 	}*/

		 	if(!DCoffSet_Calibration.getEIT_Control_Dlg().EIT_Control.MuxBoardControl(1,DcOffCal.Cal1_ch,DcOffCal.Cal2_ch)) //MuxBoardControl(1,src,sinc))
		 	{
		 		JOptionPane.showMessageDialog(null, "AnalogBackplane Switch Setting Error", "InfoBox: " + "Error", JOptionPane.INFORMATION_MESSAGE);
		 	}
		
			/*if(!DCoffSet_Calibration.getEIT_Control_Dlg().EIT_Control.TimeSlotSetting(1000000))
			{
			...
			/*if(!DCoffSet_Calibration.getEIT_Control_Dlg().EIT_Control.CommFPGASet(DCoffSet_Calibration.getEIT_Control_Dlg().EIT_Control.m_nOpMode >> 3,0,0,DCoffSet_Calibration.getEIT_Control_Dlg().EIT_Control.m_nTotalProjection-1,0))
			{
		...
		boolean m_bInitCCSGIC = false;
			...
			m_nOverflow = DCOffsetProjection(Definition.Serial_EIT_ALL_CH,m_nOverflow);
			if(m_nOverflow == null)
			{
	...
	{
		/*if(!DCoffSet_Calibration.getEIT_Control_Dlg().EIT_Control.AnalogBackplaneSWSetting(Definition.ALL_CH,1,1,0,0,0,0))	// SW1 is GND switch
		{
			JOptionPane.showMessageDialog(null, "AnalogBackplane Switch Setting Error", "InfoBox: " + "Error", JOptionPane.INFORMATION_MESSAGE);
			return false;
		}*/
		
		if(!DCoffSet_Calibration.getEIT_Control_Dlg().EIT_Control.MuxBoardControl(0,DcOffCal.Cal1_ch,DcOffCal.Cal2_ch))	// GND SW On
		{
			{
				JOptionPane.showMessageDialog(null, "AnalogBackplane Switch Setting Error", "InfoBox: " + "Error", JOptionPane.INFORMATION_MESSAGE);
				return false;
		...
		m_nOverflow = DCOffsetProjection(TotalCh,m_nOverflow);
...
/*
		...
		if(!DCoffSet_Calibration.getEIT_Control_Dlg().EIT_Control.MuxBoardControl(1,DcOffCal.Cal1_ch,DcOffCal.Cal2_ch))
		{
			{
				JOptionPane.showMessageDialog(null, "AnalogBackplane Switch Setting Error", "InfoBox: " + "Error", JOptionPane.INFORMATION_MESSAGE);
				return false;
		...
		int[] Min = new int [Definition.Serial_EIT_ALL_CH];
		int[] Max = new int [Definition.Serial_EIT_ALL_CH];
		for(int i = 0; i < Definition.Serial_EIT_ALL_CH; i++)
		{
			Max[i] = 300;
			Min[i] = -300;
		...
		DCOffsetWriteFile(Definition.Serial_EIT_ALL_CH,m_nDCOffsetValue,FilePath);
	...
	Object[] DCOffsetLoop(int[] RangeMin, int[] RangeMax, int Step,int[] OffsetValue, long[] AccumulatedData,  long[] AccumulatedGNDData) throws HeadlessException, InterruptedException, IOException
	{
		...
		int[] OffsetIndex;
		long[] Temp = new long[Definition.Serial_EIT_ALL_CH];

		TempAccumulatedData = new long[Definition.Serial_EIT_ALL_CH][Length];
		TempOffsetValue = new int[Definition.Serial_EIT_ALL_CH][Length];
		OffsetIndex = new int[Definition.Serial_EIT_ALL_CH];
		...
		Object[] Sim_data = FindAbsSimilarValue(TempAccumulatedData,Definition.Serial_EIT_ALL_CH,Length,AccumulatedGNDData,AccumulatedData,OffsetIndex); 
