...
/////////////////////////////////////////////////////
	...
	public int USBComm2(int SendLength, int RecieveLength, int NumofRecieveBlock, byte[] Data)
	{
		...
		int USBComm4(int SendLength, int RecieveLength, int NumofRecieveBlock, int[] Data)
		{
			int success = FALSE;
		
			int[] buf = new int [MAX_PACKET_SIZE_READ];
			...
			if(DeviceWrite4(buf, SendLength + 2, IBR_BytesWritten,0)==0) 
	
			{
				...
				for(int i = 0; i < NumofRecieveBlock; i++)
				{
					...
					status = DeviceRead4(buf, RecieveLength, IBR_BytesRead,0);
					...
					for(int j = 0; j < IBR_BytesRead.getValue(); j++)
					{
						m_nResponse[index] = (byte)buf[j];   //////check
						index++;
					}
				}
			}
	...
	public int  FillDeviceList()
	{
		...
		int status = USB.SI_GetNumDevices(dwNumDevices);
		...
		if (status == SiUSBXp.SI_SUCCESS)
		{
			...
			{
				status = USB.SI_GetProductString(d, m_strDeviceString, SiUSBXp.SI_RETURN_SERIAL_NUMBER);
	...
	public int USBPortOpen(int DeviceNum)
	{
		if (m_nNumofDevice > 0)
		{
			...
			m_PortOpenStatus[DeviceNum] = USB.SI_Open(DeviceNum,dev_handle_ref);
	...
	public int USBPortClose(int DeviceNum)
	{
		...
		if(m_PortOpenStatus[DeviceNum] == SiUSBXp.SI_SUCCESS)
		{
			...
			status = USB.SI_Close(dev_handle_ref);
		...
		if (DeviceWrite(msg, FT_MSG_SIZE, IBR_BytesWritten,0) ==0) ///////////////////
		{
			...
			if (DeviceRead(buf, FT_MSG_SIZE, IBR_BytesRead,0) ==0)////////////
			{
				...
				while (counterPkts < numPkts && success==1)
				{
					...
					{
					//	AfxMessageBox(_T("Failed reading file packet from target device."));
			...
			{
//				AfxMessageBox(_T("Failed reading file size message from target device."));
		...
		{
//			AfxMessageBox(_T("Failed sending read file message to target device."));
...
//check if char of byte
	public int WriteData(byte Data1, byte Data2,byte Data3,byte Data4,byte Data5,byte Data6,byte Data7,byte Data8,byte Data9,byte Data10)
	{
		...
		buf[0] = FT_WRITE_MSG;
		buf[1] = (char)(10 & 0x000000FF); /////////////// make sure not char !!!!!!!!!
	...
	int DeviceWrite(byte[] buffer, int dwSize, IntByReference lpdwBytesWritten, int dwTimeout)
	{
		int tmpReadTO=0 ,tmpWriteTO =0;				
		int	status = SiUSBXp.SI_SUCCESS;

		USB.SI_GetTimeouts(tmpReadTO, tmpWriteTO);

		USB.SI_SetTimeouts(0, dwTimeout);

		status = USB.SI_Write(dev_handle_ref.getValue(), buffer, dwSize, lpdwBytesWritten);

		USB.SI_SetTimeouts(tmpReadTO, tmpWriteTO);

		//return (status ==SiUSBXpSI_SUCCESS);
		status =SiUSBXp.SI_SUCCESS;
		return (status);
	}
	

	int DeviceRead(byte[] buffer, int dwSize, IntByReference lpdwBytesRead, int dwTimeout)
	{
		...
		if (dwTimeout == 0)
		{
			...
			{
				status = USB.SI_CheckRXQueue(dev_handle_ref.getValue(), int_dwBytesInQueue, int_dwQueueStatus);
		...
		{
			USB.SI_SetTimeouts(dwTimeout, 0);
		}

		if (status == SiUSBXp.SI_SUCCESS)
		{
			status = USB.SI_Read(dev_handle_ref.getValue(), buffer, dwSize, lpdwBytesRead);
		//	for (int i = 0 ;i <12 ;i++)
			//	System.out.printf("DeviceRead buffer  %d\n ",buffer[i]&0xFF);
		}
	...
	int DeviceWrite2(byte[] buffer, int dwSize, IntByReference lpdwBytesWritten, int dwTimeout)
	{
		
		//int tmpReadTO, tmpWriteTO;				
		int	status = SiUSBXp.SI_SUCCESS;
	
		status = USB.SI_Write(dev_handle_ref.getValue(), buffer, dwSize, lpdwBytesWritten);
		//status = USB.SI_Write(m_hUSBDevice[m_nDeviceList], buffer, dwSize, lpdwBytesWritten);
		status = SiUSBXp.SI_SUCCESS;


		return (status);
	}
	int DeviceWrite4(int[] buffer, int dwSize, IntByReference lpdwBytesWritten, int dwTimeout)
	{
	...
	int DeviceRead2(byte[] buffer, int dwSize, IntByReference lpdwBytesRead, int dwTimeout)
	{
		//int tmpReadTO, tmpWriteTO;					// Current timeout values.
		int	status = SiUSBXp.SI_SUCCESS;
		int dwQueueStatus	= SiUSBXp.SI_RX_NO_OVERRUN;
		int dwBytesInQueue = 0;
		//int lpdwBytesInQueue ;
		...
		if (status == SiUSBXp.SI_SUCCESS)
		{
			...
			//for (int i = 0 ;i <12 ;i++)
			//System.out.printf("DeviceRead2 buffer  %d\n ",buffer[i]&0xFF);
	...
	int DeviceRead3(byte[] buffer, int dwSize, IntByReference lpdwBytesRead, int dwTimeout)
	{
		int	status = SiUSBXp.SI_SUCCESS;
		int dwQueueStatus	= SiUSBXp.SI_RX_NO_OVERRUN;
		int dwBytesInQueue = 0;
	...
	int DeviceRead4(int[] buffer, int dwSize, IntByReference lpdwBytesRead, int dwTimeout)
	{
		int	status = SiUSBXp.SI_SUCCESS;
		int dwQueueStatus	= SiUSBXp.SI_RX_NO_OVERRUN;
		int dwBytesInQueue = 0;
		...
		if (status == SiUSBXp.SI_SUCCESS)
		{
			//		status = SI_Read(m_hUSBDevice[m_nDeviceList], buffer, dwSize, lpdwBytesRead);
			status = USB.SI_Read(dev_handle_ref.getValue(), buffer, dwSize, lpdwBytesRead);
