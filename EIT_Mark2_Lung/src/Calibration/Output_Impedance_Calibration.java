	...
	public Output_Impedance_Calibration(Control control) {
		...
		btnCalibrateAll.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				...
				int []  CoarseR = new int[Definition.TOTAL_CH];
				int []  FineR = new int[Definition.TOTAL_CH];
				int []  CoarseC = new int[Definition.TOTAL_CH];
				int []  FineC = new int[Definition.TOTAL_CH];
				try {
					if(!DC_Cal_fun.CCSCalSystemSetting())
					{
						...
						DC_Cal_fun.CCSCalWriteFile(Definition.TOTAL_CH,DC_Cal_fun.m_strFilePath,FineR,CoarseR,FineC,CoarseC);
		...
		btnCalibrate.setBounds(664, 75, 108, 25);
		panel.add(btnCalibrate);
