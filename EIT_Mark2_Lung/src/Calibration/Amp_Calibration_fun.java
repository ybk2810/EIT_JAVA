...
public class Amp_Calibration_fun {
	 SiUSBXp USB_lib = SiUSBXp.INSTANCE;
	  ...
	  double[] Magnitude = new double [Definition.TOTAL_CH];
	  double[] Phase = new double [Definition.TOTAL_CH];
	  ...
	  boolean AmpCalSystemSetting() throws HeadlessException, InterruptedException
	  {
	  	...
	  	if(!Amplitude_Calibration.getEIT_Control_Dlg().EIT_Control.CommFPGASet(Amplitude_Calibration.getEIT_Control_Dlg().EIT_Control.m_nOpMode >> 3,0,0,Amplitude_Calibration.getEIT_Control_Dlg().EIT_Control.m_nTotalProjection-1))
	  	{
		...
		Gap= FreqtoCNTGap_data[2];	
	  	...
	  	if(!AmpCalProjection(Definition.TOTAL_CH))
	  	{
	  		return false;
	  	}
